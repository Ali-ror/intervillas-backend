<template>
  <table class="table border-light border-bottom">
    <tbody v-if="showBreakdown && villa.length" class="villa-clearing-details">
      <tr>
        <th
            class="text-primary"
            colspan="2"
      ...
      <tr v-for="item in villa.rentItems" :key="item.id">
      ...
      <tr v-for="item in villa.chargeItems" :key="item.id">
        <td class="border-top-0" v-text="item.human_category" />
        <td class="border-top-0 text-right" v-text="currency(item.price)" />
      </tr>
      <tr v-for="item in villa.otherItems" :key="item.id">
        <td class="border-top-0" v-text="item.human_category" />
        <td class="border-top-0 text-right" v-text="currency(item.price)" />
      </tr>
    </tbody>

    <tbody v-if="showBreakdown && boat.length " class="boat-clearing-details">
      <tr>
        <th
            class="text-primary"
            colspan="2"
            v-text="t('boat_prices')"
        />
      </tr>
      <tr v-for="item in boat.rentItems" :key="item.id">
        <td class="border-top-0" v-text="item.human_category" />
        <td class="border-top-0 text-right" v-text="currency(item.price)" />
      </tr>
      <tr v-for="item in boat.chargeItems" :key="item.id">
        <td class="border-top-0" v-text="item.human_category" />
        <td class="border-top-0 text-right" v-text="currency(item.price)" />
      </tr>
      <tr v-for="item in boat.otherItems" :key="item.id">
        <td class="border-top-0" v-text="item.human_category" />
        <td class="border-top-0 text-right" v-text="currency(item.price)" />
      </tr>
    </tbody>
...
class Rentable {
  constructor(type, { rentItems, chargeItems, otherItems }) {
    const f = it => it.rentable === type
    this.rentItems = rentItems.filter(f)
    this.chargeItems = chargeItems.filter(f)
    this.otherItems = otherItems.filter(f)
  }

  get length() {
    return this.rentItems.length + this.chargeItems.length + this.otherItems.length
  }
}
...
export default {
  ...
  computed: {
    villa() {
      return new Rentable("villa", this)
    },

    boat() {
      return new Rentable("boat", this)
    },
      ...
      return this.clearing.rentable_clearings.reduce((rents, curr) => {
        for (let i = 0, len = curr.rents.length; i < len; ++i) {
          const rent = curr.rents[i],
                { category, villa_id, boat_id } = rent,
                rentable = villa_id ? "villa" : boat_id ? "boat" : "?"

          if (RENT_CATEGORIES.includes(category)) {
            const { human_category, human_time_units, time_units, start_date, total } = rent
            rents.push({
              id:    `${category}/${rentable}/${start_date}/${time_units}`,
              category,
              human_category,
              rentable,
              human_time_units,
      ...
      return Object.values(this.clearing.rentable_clearings.reduce((group, curr) => {
        for (let i = 0, len = curr.rents.length; i < len; ++i) {
          const charge = curr.rents[i],
                { category, villa_id, boat_id } = charge,
                rentable = villa_id ? "villa" : boat_id ? "boat" : "?",
                [key, human_category] = surchargeName(category)

          if (key) {
            const { start_date, total } = charge,
                  id = `${key}/${rentable}/${start_date}`

            group[id] || (group[id] = { id, human_category, rentable, price: 0 })
