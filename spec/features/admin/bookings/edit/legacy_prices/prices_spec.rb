  ...
  shared_examples "Preise ändern" do
    feature "Preise ändern" do
      ...
      scenario do
        ...
        fill_in_clearing_item "Erwachsene", with: 99

        new_adult_price = find("#price_table tr", text: "Erwachsene").find("td.text-right:last-child").text
        expect(new_adult_price).to eq curr_values.lookup("7 Nächte 2 Erwachsene", :custom_prices)[1]
        ...
        within_price_table do
          expect_price_row_field "Erwachsene", with: 99
          expect_price_row_field "Bearbeitungsgebühr", with: 50
        end

        within_price_table do
          expect_price_row_value_lookup("7 Nächte 2 Erwachsene", :custom_prices)
